...
@php
  ...
  $guestBrandLogo = $guestTenant?->branding_path ? asset($guestTenant->branding_path) : asset('assets/img/logo-ct.png');
@endphp
<nav class="navbar navbar-expand-lg position-absolute top-0 z-index-3 my-3 {{ (Request::is('static-sign-up') ? 'w-100 shadow-none  navbar-transparent mt-4' : 'blur blur-rounded shadow py-2 start-0 end-0 mx4') }}">
  <div class="container-fluid {{ (Request::is('static-sign-up') ? 'container' : 'container-fluid') }}">
    <a class="navbar-brand font-weight-bolder ms-lg-0 ms-3 d-flex align-items-center gap-2 {{ (Request::is('static-sign-up') ? 'text-white' : '') }}" href="{{ url('dashboard') }}" data-testid="guest-tenant-brand">
      <img src="{{ $guestBrandLogo }}" alt="Logo {{ $guestBrandName }}" height="24" class="rounded bg-white p-1">
      <span>{{ $guestBrandName }}</span>
    </a>
